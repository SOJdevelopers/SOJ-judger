...
{
    setName("compare two sequences of doubles, max absolute error = %.10lf", EPS);
    ...
    while (!ans.seekEof()) 
    {
        ...
        {
            quitf(_wa, "%d%s numbers differ - expected: '%.10lf', found: '%.10lf', error = '%.10lf'",
                n, englishEnding(n).c_str(), j, p, doubleAbsoluteDelta(j, p));
        }
    }

    if (n == 1)
        quitf(_ok, "found '%.10lf', expected '%.10lf', error '%.10lf'", p, j, doubleAbsoluteDelta(j, p));
