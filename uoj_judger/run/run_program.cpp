...
{
	{"tl"                 , 'T', "TIME_LIMIT"  , 0, "Set time limit (in float second)"                      ,  1},
	{"rtl"                , 'R', "TIME_LIMIT"  , 0, "Set real time limit (in float second)"                 ,  2},
...
void parse_args(int argc, char **argv) {
	...
	if (run_program_config.type == "java8u282") {
	...
	} else if (run_program_config.type == "python3.8") {
		string pre[3] = {"/usr/bin/python3.8", "-I", "-B"};
		run_program_config.argv.insert(run_program_config.argv.begin(), pre, pre + 3);
	} else if (run_program_config.type == "java8u282") {
		string pre[3] = {abspath(0, string(self_path) + "/usr/bin/java"), "-Xmx1024m", "-Xss1024m"};
		run_program_config.argv.insert(run_program_config.argv.begin(), pre, pre + 3);
	}
}
...
void run_child() {
	set_limit(RLIMIT_CPU, (run_program_config.time_limit + 1000) / 1000, (run_program_config.real_time_limit + 3000) / 1000);
...
RunResult trace_children() {
	...
	if (rp_timer_pid == -1) {
		...
		struct timespec ts;
		ts.tv_sec = run_program_config.real_time_limit / 1000 + 2;
		ts.tv_nsec = run_program_config.real_time_limit % 1000 * 1000000;
		nanosleep(&ts, NULL);
		exit(0);
	}
