...
void init_conf(const RunProgramConfig &config) {
	...
	if (config.type == "python2") {
		...
		readable_file_name_set.insert("/sys/devices/system/cpu/");
		readable_file_name_set.insert("/proc/");
		writable_file_name_set.insert("/proc/self/coredump_filter");
		# ifdef UOJ_JUDGER_BASESYSTEM_UBUNTU1804
		readable_file_name_set.insert("/sys/fs/cgroup/cpu/");
		readable_file_name_set.insert("/sys/fs/cgroup/cpu,cpuacct/");
		readable_file_name_set.insert("/sys/fs/cgroup/memory/");
...
inline bool check_safe_syscall(pid_t pid, bool need_show_trace_details) {
	...
	if (syscall_should_soft_ban[syscall]) {
		...
		if (need_show_trace_details) fprintf(_debug, "dgs      %d\n", (int)syscall), fclose(_debug);
		return false;
	}
	...
	if (syscall == __NR_open || syscall == __NR_openat) {
		...
		if (is_read_only) {
			if (realpath(fn) != "" && !is_readable_file(fn)) {
				if (need_show_trace_details) fputs("dangerous R\n", _debug), fclose(_debug);
				return on_dgs_file_detect(pid, reg, fn);
			}
		} else {
			if (!is_writable_file(fn)) {
				if (need_show_trace_details) fputs("dangerous W\n", _debug), fclose(_debug);
				return on_dgs_file_detect(pid, reg, fn);
			}
		}
		...
		string fn = read_abspath_from_regs(fn_addr, pid);
		if (need_show_trace_details) {
			fprintf(_debug, "readlink %s\n", fn.c_str());
		}
		if (!is_readable_file(fn)) {
			if (need_show_trace_details) fputs("dangerous L\n", _debug), fclose(_debug);
			return on_dgs_file_detect(pid, reg, fn);
		}
		...
		string fn = read_abspath_from_regs(fn_addr, pid);
		if (need_show_trace_details) {
			fprintf(_debug, "unlink   %s\n", fn.c_str());
		}
		if (!is_writable_file(fn)) {
			if (need_show_trace_details) fputs("dangerous U\n", _debug), fclose(_debug);
			return on_dgs_file_detect(pid, reg, fn);
		}
		...
		string fn = read_abspath_from_regs(fn_addr, pid);
		if (need_show_trace_details) {
			fprintf(_debug, "access   %s\n", fn.c_str());
		}
		if (!is_statable_file(fn)) {
			if (need_show_trace_details) fputs("dangerous A\n", _debug), fclose(_debug);
			return on_dgs_file_detect(pid, reg, fn);
		}
		...
		string fn = read_abspath_from_regs(fn_addr, pid);
		if (need_show_trace_details) {
			fprintf(_debug, "stat     %s\n", fn.c_str());
		}
		if (!is_statable_file(fn)) {
			if (need_show_trace_details) fputs("dangerous S\n", _debug), fclose(_debug);
			return on_dgs_file_detect(pid, reg, fn);
		}
		...
		string fn = read_abspath_from_regs(fn_addr, pid);
		if (need_show_trace_details) {
			fprintf(_debug, "execve   %s\n", fn.c_str());
		}
		if (!is_readable_file(fn)) {
			if (need_show_trace_details) fputs("dangerous E\n", _debug), fclose(_debug);
			return on_dgs_file_detect(pid, reg, fn);
		}
		...
		string fn = read_abspath_from_regs(fn_addr, pid);
		if (need_show_trace_details) {
			fprintf(_debug, "change   %s\n", fn.c_str());
		}
		if (!is_writable_file(fn)) {
			if (need_show_trace_details) fputs("dangerous C\n", _debug), fclose(_debug);
			return on_dgs_file_detect(pid, reg, fn);
		}
	}
	if (need_show_trace_details) fclose(_debug); 
